    ...
    try {
      ...
      let _prefix = plugin.customPrefix ? plugin.customPrefix : conn.prefix ? conn.prefix : global.prefix
        ...
        if (!isPrems && global.DATABASE._data.users[m.sender].limit < 1 && plugin.limit) {
          this.reply(m.chat, `Seu limite aumentou, por favor, compre via *${usedPrefix}Comprar*`, m)
          continue
        }
        try {
          await plugin(m, {
          ...
          if (m.limit == true) this.reply(m.chat, '1 Limites se aplicam', m)
        }
  			break
  		}
  	}
...
global.mods = [5533988704560]
global.prems = [5511946817667]
