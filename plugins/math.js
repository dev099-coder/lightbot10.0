...
let handler  = async (m, { conn, args, usedPrefix }) => {
  if (args.length < 1) return conn.reply(m.chat, 'Modo: novato | fácil | médio | Difícil | extremo\n\nExemplos de uso: ' + usedPrefix + 'matemática médio', m)
  let mode = args[0].toLowerCase()
  if (!(mode in modes)) return conn.reply(m.chat, 'Modo: novato | fácil | médio | Difícil | extremo\n\nExemplos de uso: ' + usedPrefix + 'matemática médio', m)
  let id = m.chat
  if (id in global.math) return conn.reply(m.chat, 'Ainda há perguntas sem resposta neste chat', global.math[id][0])
...
handler.help = ['matemática <modo>']
handler.tags = ['game']
handler.command = /^matemática/i
