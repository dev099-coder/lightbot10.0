...
let handler  = async (m, { conn, args, usedPrefix, command }) => {
  ...
  if ((args[0] && args[0] == 'plz') || global.conn.user.jid == conn.user.jid) {
    ...
    conn.once('connection-validated', user => {
      parent.reply(m.chat, 'Conectado com sucesso ao WhatsApp - mu.\n*NOTE: Isso é só passeio*\n' + JSON.stringify(user, null, 2), m)
    })
    conn.on('message-new', global.conn.handler)
    conn.regenerateQRIntervalMs = null
    conn.connect().then(async ({user}) => {
      if (auth) return
      await parent.sendMessage(user.jid, `Você pode fazer o login sem qr com a mensagem abaixo. para obter o código completo, por favor envie para obter um código preciso`, MessageType.extendedText)
